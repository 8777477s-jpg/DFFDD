...
- **SmartRuleEvaluator**: deterministic score combiner and decision policy (threshold + stability + cooldown + observation freshness decay).
...
4. Controller updates per-rule runtime signal cache and applies optional foreground-window safety gate.
...
- Timeline event with score + top reason summary.
