...
public sealed class Controller : IDisposable
{
    ...
    private void HandleObservation(string ruleId, Observation observation)
    {
        try
        {
            var rule = _storage.ListRules().FirstOrDefault(x => x.Id == ruleId);
    ...
    public void ApplyRuleFeedback(string ruleId, bool wasCorrect)
    {
        ...
        SmartRuleEvaluator.ApplyFeedback(rule, _runtimeCache.ForRule(ruleId), wasCorrect);
        _storage.UpsertRule(rule);
        _storage.InsertRuleFeedback(new RuleFeedbackRecord { RuleId = ruleId, WasCorrect = wasCorrect });
        _timeline.Add(new TimelineEvent { Source = TimelineSource.Rule, Message = $"[SmartRules] Feedback ruleId={ruleId} result={(wasCorrect ? "correct" : "false-trigger")} threshold={rule.Smart.FireThreshold:0.00} weights=({rule.Smart.RoiWeight:0.00},{rule.Smart.UiaWeight:0.00},{rule.Smart.OcrWeight:0.00})" });
    }
